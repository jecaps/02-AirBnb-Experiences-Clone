export default function Article(props) {
  return (
    <article className="article">
      <img className="article--img" src={props.el.imageUrl} alt="Castle" />
      <div className="article--details">
        <div className="article--map">
          ...
          <p>{props.el.location}</p>
          <a href={props.el.googleMapsUrl}>View on Google Maps</a>
        </div>
        <h3 className="article--title">{props.el.title}</h3>
        <div>
          <p className="article--date">
            {props.el.startDate} - {props.el.endDate}
          </p>
          <p className="article--description">{props.el.description}</p>
        </div>
      </div>
    </article>
  );
